...
# 조건에 맞는 레시피를 검색하여 해당 레시피아이디를 반환하는 메소드.

def input_information(humanSentence):
    cursor = connection.cursor()

    humanSentence
    if ("/" not in humanSentence &","in humanSentence):  # 재료만 입력한다고 버튼을 선택했을 경우
        ...
        IngredientSelect = "SELECT recipe_id,"
        IngredientSelect_sub = " FROM searching s ,mainrecipe m WHERE s.recipe_id = m.recipe_id "

        for j in hateList:
            HateIngredient += "searching_ingredients NOT LIKE '%" + j.strip() + "%' AND "

        for i in ingredient:
            IngredientScore += "(searching_ingredients LIKE '%" + i.strip() + "%')+"  #
        IngredientScore = IngredientScore.rstrip('+')  # 맨마지막 재료뒤에도 붙어버린 +제거
        IngredientScore += ")/ingredient_num"
        IngredientSelect_sub += HateIngredient + IngredientScore + ">=0.8 ORDER BY ingredient_score DESC, m.importance DESC"
    ...
    elif ("," not in humanSentence & "/" in humanSentence):  # 요리명만 입력한다고 버튼을 선택했을 경우
    ...
    elif ("," in humanSentence & "/" in humanSentence):  # 요리명과 제목을 입력한다고 버튼을 선택했을 경우
...
def create_view():
    ...
    createView_sql = "CREATE OR REPLACE VIEW v_onlySearching AS SELECT cooking_title, cooking_steps, cooking_tips" \
                     ", cooking_time, cooking_level, importance, match_ingredient FROM mainrecipe WHERE " + view_id
    cursor.execute(createView_sql)
    createView_sql = "CREATE OR REPLACE VIEW v_onlySearching_ingre AS SELECT recipe_id, ingredient,amount*"+result_servings+" FROM recipe_ingredient WHERE " + view_id
    cursor.execute(createView_sql)
    for i in range(len(result_info)):
        matchUpdate_sql = "UPDATE v_onlySearching SET match_ingredient = " + result_info[i]['ingredient_score'] + "" \
                                                                                                                  " WHERE recipe_id = " + \
                          result_info[i]['recipe_id']
...
def select_info():
    cursor = connection.cursor()
    info_sql = "SELECT cooking_title, cooking_steps, cooking_tips, cooking_time, cooking_level, " \
               "importance, match_ingredient from v_onlySearching"
    cursor.execute(info_sql)
    final_information = cursor.fetchall()
...
if __name__ == "__main__":
    ...
    botSentence = '안녕 나는 냠냠봇이야!^^ 찾는 레시피 있니?\n ' \
        ...
        # 처음 챗봇이 할 말.
    while True:
        print("1. 재료   2. 요리명   3. 재료+요리명 \n")
        humanSentence = input('you> ').strip()
        a = inputType_check(int(humanSentence))
        print(a)
        print("당신이 선택한 번호.", status_info+1)
        humanSentence_ing = input('you> ').strip()

        print("몇인분? \n")
        humanSentence = input('you> ').strip()
        servings_check(humanSentence)
        print("당신이 선택한 인분", servings_check(humanSentence))

        hateList = "당근"

        print(input_information(humanSentence_ing))
        print(result_info)
